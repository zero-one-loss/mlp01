...
class Substitute(object):
    ...
    def predict(self, x, batch_size):
        ...
        return torch.from_numpy(self.model.predict(x, kind='best'))
    ...
    def train(self, x, y, batch_size, n_epoch):
        self.model = SCD(nrows=0.75, nfeatures=1, w_inc=0.17, b_ratio=0.2,
                         tol=0.000000, local_iter=1, num_iters=1000,
                         updated_features=128, round=4,
                         interval=20, n_jobs=4, num_gpus=4,
                         width=6400, verbose=False)
...
def get_data(train_size=200):
    ...
    if args.dataset == 'mnist':
        ...
        sub_x = data[indices[:train_size]].float().reshape((-1, 28 * 28))
        sub_x /= 255
        test_data = data[indices[train_size:]].float().reshape((-1, 28 * 28))
        test_data /= 255
...
def jacobian_augmentation(model, x_sub, y_sub, Lambda, samples_max):
    ...
    train_data_adv[:, model.model.best_w_index] -= Lambda * (yp * 2 - 1)
...
def get_adv(scd, test, test_label, epsilon):


    test_data_adv = np.zeros_like(test)
    # yp = scd.predict(test, kind='best')
    test_data_adv[:, scd.model.best_w_index] -= epsilon * np.sign(scd.model.best_w.cpu().numpy().reshape((1, -1))) \
                                          * (test_label.numpy().reshape((-1, 1)) * 2 - 1)
    test_data_adv += test.numpy()
    np.clip(test_data_adv, 0, 1, out=test_data_adv)

    return torch.from_numpy(test_data_adv)
...
def stl10_bbox_sub(param, oracle_model, substitute_model, x_sub, test_data, \
    ...
    for rho in range(aug_epoch):
        ...
        test_adv = get_adv(substitute_model, test_data, test_label, param['epsilon'])
        ...
        print('Oracle model FGSM attack\'s accuracy on adversarial samples #%d:' % test_adv.shape[0])
        ...
        # print(type(yp_original))
        # print(type(sp_original))
