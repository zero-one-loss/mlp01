...
from .scd01_binary import SCD as SMP
class SCD(object):
    def __init__(self, n_classes=10, smp_params=None):
        self.n_classes = n_classes
        self.n_clf = n_classes
        self.scds = {}
    ...
    def predict(self, data, kind='vote', all=False, bs=False, distance=False):
        # if bs:
        ...
        if distance:
            ...
            for i in range(self.n_classes):
                yp[:, i] = self.scds['%s' % i].predict_projection(data, kind='vote', all=True).sum(axis=1)
    ...
    def bs(self, data, label):
        ...
        for i in range(self.n_classes):
            yp[:, :, i] = self.scds['%s' % i].predict(data, kind='vote', all=True)
        data_prob = yp.reshape((yp.shape[0], -1))
        self.mlp = MLPClassifier(hidden_layer_sizes=(30,), activation='logistic', solver='sgd', alpha=0.0001,
                                    batch_size='auto',
